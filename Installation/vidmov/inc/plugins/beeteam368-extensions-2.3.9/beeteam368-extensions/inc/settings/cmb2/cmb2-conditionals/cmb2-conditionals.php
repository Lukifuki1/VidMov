...
if (!class_exists('CMB2_Conditionals', false)) {
    ...
    class CMB2_Conditionals
    {
        ...
        public function admin_footer()
        {
            ...
            if ($enqueue_script) {
                ...
                wp_enqueue_style(
                    'cmb2-conditionals',
                    plugins_url('/cmb2-conditionals.css', __FILE__),
                    array(),
                    self::VERSION
                );/*add by BeeTeam368*/
            }
        ...
        public function filter_data_to_save(CMB2 $cmb2, $object_id)
        {
            foreach ($cmb2->prop('fields') as $field_args) {
                if (!('group' === $field_args['type'] || (array_key_exists('attributes', $field_args) && is_array($field_args['attributes']) && array_key_exists('data-conditional-id', $field_args['attributes'])))) {
                    continue;
                }
    ...
    if ((function_exists('wp_installing') && wp_installing() === false) || (!function_exists('wp_installing') && (!defined('WP_INSTALLING') || WP_INSTALLING === false))) {
        add_action('plugins_loaded', 'cmb2_conditionals_init', 20, 1);
    }
